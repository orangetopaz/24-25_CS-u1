...
string getTypeName(const variant<int, float, string, bool>& var) {  // THIS FUNCTION IS GENERATED BY CHAT GPT, I ONLY UNDERSTAND HOW SOME OF IT WORKS. DO NOT GRADE THIS.
...
void printAny(variant<int, float, string, bool> toPrint){
...
typedef vector<vector<variant<int, float, string, bool>>> listolists_t;  // this just makes it so I don't have to type out that whole thing every time
...
void printListolists(listolists_t listolists){
  cout << "{";
  for(vector<variant<int, float, string, bool>> i : listolists){
    cout << "{";
    for(variant<int, float, string, bool> j : i){
      printAny(j);
      cout << ", ";
    }
    cout << "}";
